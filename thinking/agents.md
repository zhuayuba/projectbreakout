...
另眼相看是破茧计划的姊妹系列。核心思路：**用"学科季"模式，像物理学家、经济学家、历史学家……一样思考**。每季聚焦一个学科，每季 4 期，每期讲透该学科的一种底层思维习惯。目标是用大白话说清楚"像XX家一样思考"到底意味着什么。
...
破茧计划使用 `munger_*` 前缀，两者完全隔离。

## 每期内容模板

每期 HTML 文件遵循固定结构，但与破茧计划不同——重点不是"硬科学模型"，而是**学科底层思维习惯**：
...
#### 坑 5：配色与破茧计划太像
**现象**：最初使用深蓝 hero + gold accent → 和破茧计划几乎一样。换成深绿 → 差别仍然不够大。
...
- 按钮链接到 `https://zhuayuba.github.io/projectbreakout/thinking/NN.html`
